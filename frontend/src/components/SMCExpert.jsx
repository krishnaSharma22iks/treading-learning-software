...
const SMCExpert = ({ smc }) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-black/40 backdrop-blur-3xl border border-white/5 rounded-[2rem] p-6 shadow-2xl overflow-hidden relative group"
    >
      {/* Glow effect */}
      <div className={`absolute top-0 right-0 w-32 h-32 blur-[80px] -mr-16 -mt-16 opacity-20 pointer-events-none rounded-full ${smc.decision === "ENTER" ? "bg-emerald-500" : "bg-white/10"}`}></div>
      ...
      <div className="relative z-10 flex flex-col gap-6">
        <div className="flex items-center justify-between">
          ...
          <div className={`px-3 py-1 rounded-lg text-[10px] font-black border tracking-widest ${smc.decision === "ENTER" ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-white/5 text-slate-400 border-white/10'}`}>
            {smc.decision}
          </div>
        </div>
